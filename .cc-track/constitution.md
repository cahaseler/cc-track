...
**Version**: 1.2.1
**Ratified**: 2025-10-15
**Last Amended**: 2025-12-06 (Replaced test count requirement with coverage requirement)
...
- Per-project configuration in `.cc-track/track.config.json`
...
- [ ] All tests passing with 80%+ line coverage
...
- **Coverage**: 80% minimum line coverage (check with `bun test --coverage`)
...
- See `.cc-track/code_index.md` for current directory structure and key files
- Code index must be kept accurate and up-to-date as project evolves
...
- **Configuration**: Per-project `.cc-track/track.config.json`
...
- **Persistent Context**: Core principles in product_context.md, system_patterns.md, decision_log.md (stored in `.cc-track/`)
- **Active Tasks**: Spec folders in `.cc-track/specs/NNN-feature-name/`
- **Configuration**: Per-project `.cc-track/track.config.json` enables/disables features
- **Workflow**: Spec-driven (/cc-track:specify → /cc-track:plan → /cc-track:tasks)
...
- **Mechanism**: `.cc-track/track.config.json` in each project
...
### Related Documents
- **System Patterns**: `.cc-track/system_patterns.md` - Implementation patterns and conventions
- **Decision Log**: `.cc-track/decision_log.md` - Historical architectural decisions
- **Product Context**: `.cc-track/product_context.md` - Product vision and goals
- **Workflow Guide**: `.cc-track/cc-track-workflow.md` - Development workflow
