...
- [2025-09-15] Fix pushCurrentBranch to handle diverged branches automatically. The complete-task command often fails to push when semantic-release has added commits to the remote branch. Solution: Update GitHubHelpers.pushCurrentBranch() to: 1) Fetch first to detect divergence, 2) If branches have diverged, attempt `git pull --rebase`, 3) If rebase succeeds (common case with release commits), continue with push, 4) If rebase has conflicts, abort rebase and fail with clear error message for manual resolution. This will handle the common case of release commits automatically while still failing safely when there are real conflicts.
