...
## Direction Change: Temporary Workaround Implementation

After implementing the SessionStart hook solution, discovered that the approach doesn't work - environment variables set in the hook process don't persist to slash command execution. The hook runs in its own process and exits before slash commands execute.

### Decision: Implement Temporary Workaround

Rather than wait for Anthropic to fix the upstream `${CLAUDE_PLUGIN_ROOT}` bug, implemented temporary workaround by replacing script execution with manual CLI instructions in slash commands:

**Updated Commands (replaced `!` script execution with instructions):**
1. **add-to-backlog.md** - Simple file append with timestamp
2. **prepare-completion.md** - Instructions to run validation suite + code review
3. **complete-task.md** - Step-by-step git operations + PR creation
4. **migrate.md** - Active task migration only (defers historical migration)

**Key Changes:**
- Removed dependency on `${CC_TRACK_SCRIPTS_ROOT}` environment variable
- Replaced bash script execution with natural language instructions
- Instructions are project-agnostic - Claude identifies appropriate commands
- Complex error handling replaced with "explain and stop" approach
- PR description auto-generated by Claude (not reminded to user)

**Files Deleted:**
- `hooks/lib/platform-paths.ts` - No longer needed
- `hooks/lib/platform-paths.test.ts` - No longer needed
- `hooks/session-start.ts` - Approach didn't work

**Files Reverted:**
- `hooks/hooks.json` - Removed SessionStart hook registration

### Trade-offs

**Pros:**
- Works immediately without waiting for Anthropic fix
- More flexible - Claude can adapt to different project setups
- Simpler mental model - no hidden automation
- Better error messages - Claude explains what went wrong

**Cons:**
- Slightly slower than automated script execution
- Less convenient for complex operations (migrate, complete-task)
- No automatic error handling/rollback for git operations

---

## Next Steps

1. Commit workaround changes
2. Create PR and merge to main
3. Wait for Anthropic to fix `${CLAUDE_PLUGIN_ROOT}` in slash commands
4. Consider reverting to script execution once bug is fixed (optional - instructions may be better UX)
