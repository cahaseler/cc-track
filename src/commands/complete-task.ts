...
import { getCurrentBranch, getTaskBranch, isGitRepository } from '../lib/git-helpers';
...
async function squashWipCommits(taskId: string): Promise<void> {
  try {
    ...
    const lastNonWip = execSync(`git log --format="%H %s" | grep -v "\\[wip\\]" | head -1 | cut -d' ' -f1`, {
      encoding: 'utf-8',
    }).trim();
    ...
    const wipCount = parseInt(
      execSync(`git log ${lastNonWip}..HEAD --format="%s" | grep -c "\\[wip\\]" || echo 0`, {
        encoding: 'utf-8',
      }).trim(),
      10,
    );
...
export const completeTaskCommand = new Command('complete-task')
  ...
  .action(async (options) => {
    try {
      ...
      claudeContent = claudeContent.replace(/@\.claude\/tasks\/TASK_\d+\.md/, '@.claude/no_active_task.md');
