...
export class SessionReviewer {
  ...
  buildReviewPrompt(
    ...
    const prompt = `You are reviewing a portion of an AI assistant's work on a coding task. Analyze if the work is on track or has deviated.
${docNote}
...
- Things like fixes to types, security patches, lint fixes, etc, are reasonable, especially if related to the code in the task.
- This is only a portion of the work on the task, your job is to flag places where the work is exceeding the scope or shortcuts are being taken.
- Refactors are sometimes part of supporting new functionality and may be in scope.${isCompressed ? '\n- The diff has been compressed into summaries to save tokens - focus on the high-level changes described' : ''}
...
Be strict about deviations - if the changes don't directly show work towards the task requirements, it's a deviation.
